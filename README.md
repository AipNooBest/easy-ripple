...
Открываем каталог **/var/www/html/**. Теперь это будет наш основной каталог. Общий вид любого файла из /html/ можете посмотреть в файлах репозитория. Это конфигурации DRV. Так что поглядывайте иногда в качестве примера, разумеется немного переписывая строки под себя (т.е. свои домены, порты и т.д.) **ВНИМАНИЕ** **Не пытайтесь копировать файлы из репозитория в надежде на их работу. Это только в качестве примера оформления файлов!**
```
cd /var/www/html/
```
