...
class Chart:
    ...
    def before_after_dict(name, values, none_value="0"):
        ...
        """
        return {
            "{name}{'Before' if i == 0 else 'After'}": x if x is not None else none_value for i, x in enumerate(values)
        }
...
class BeatmapChart(Chart):
    ...
    def __init__(self, old_score, new_score, beatmap_id):
        ...
        super(BeatmapChart, self).__init__("beatmap", "{serverurl}/b/{beatmap_id}", "Beatmap Ranking")
...
class OverallChart(Chart):
    ...
    def __init__(self, user_id, old_user_stats, new_user_stats, score, new_achievements, old_rank, new_rank):
        ...
        """
        serverurl = glob.conf.config["server"]["serverurl"]
        super(OverallChart, self).__init__("overall", "https://{serverurl}/u/{user_id}", "Overall Ranking")
